  ...
  <div>
      ...
      <table class="table table-striped table-hover text-left">
        ...
        <tbody>
          <tr v-for="(history, index) in allHistory" :key="index">
            ...
            <td>
              <div class="d-inline px-1 text-dark" @click="edithistory(history)">Edit</div>
              <div class="d-inline px-1 text-primary" @click="deletehistory(history)">Delete</div>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    >
      <div class="d-block text-center">
        <p>
          Are you sure you want to delete this history post? ( there's no turning back
          )
        </p>
      </div>
      <div class="d-block text-right pt-3">
    ...
    >
      ...
      <b-form @submit.prevent>
        ...
        <b-form-group id="main" label="Main" label-for="main" class="text-left">
          <wysiwyg id="main" v-model="history.main" :value="currentHistory.main"></wysiwyg>
        </b-form-group>
        ...
        <div class="row">
          ...
          >
            ...
            />
            <div :class="{'selected' : isSelected(index), 'mask' :isSelected(index)}"></div>
            <div :class="{'selected' : isSelected(index)}" class="delete-text">Delete</div>
          </div>
        </div>
        <!-- gallery view  -->
      </b-form>
...
export default {
  ...
  methods: {
    ...
    edithistory(history) {
      ...
      this.history.image = this.currentHistory.image;
      this.history.title = this.currentHistory.title;
      this.history.main = this.currentHistory.main;
      this.history.comments = this.currentHistory.comments;
      this.history.excerpt = this.currentHistory.excerpt;
      this.history.gallery = this.currentHistory.gallery;
      this.history.imageId = this.currentHistory.imageId;
    },
    editOnCancel() {
    ...
    editOnConfirm() {
      ...
      let image = this.history.image,
        ...
        gallery = this.history.gallery,
        imageId = this.history.imageId;
